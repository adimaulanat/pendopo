...
int texRoof, texFloor, texPilar, texWall, texAlas;
cJSON *objPendopo;
// ----------------------------------------------------------
// Pendopo Variables
// ----------------------------------------------------------
...
void lantai(){
	zLantai = xLantai;
	glBindTexture(GL_TEXTURE_2D,texAlas);
	glBegin(GL_POLYGON);
	glColor3f( 3.0, 4.0, 0.0 );
  	glTexCoord2f(2,0);glVertex3f(  xLantai, -yLantai, -0.5 );  
  	glTexCoord2f(2,2);glVertex3f(  xLantai,  yLantai, -0.5 );
  	glTexCoord2f(0,2);glVertex3f( -xLantai,  yLantai, -0.5 );
  	glTexCoord2f(0,0);glVertex3f( -xLantai, -yLantai, -0.5 );      
  ...
  glBegin(GL_POLYGON);
  glColor3f(   1.0,  1.0, 1.0 );
  glTexCoord2f(7,0);glVertex3f(  xLantai, -yLantai, zLantai );
  glTexCoord2f(7,7);glVertex3f(  xLantai,  yLantai, zLantai );
  glTexCoord2f(0,7);glVertex3f( -xLantai,  yLantai, zLantai );
  glTexCoord2f(0,0);glVertex3f( -xLantai, -yLantai, zLantai );
  glEnd();
...
//   Purple side - RIGHT
  ...
  glBegin(GL_POLYGON);
  glColor3f(  1.0,  0.0,  1.0 );
  glTexCoord2f(7,0);glVertex3f( xLantai, -yLantai, -0.5 );
  glTexCoord2f(7,7);glVertex3f( xLantai,  yLantai, -0.5 );
  glTexCoord2f(0,7);glVertex3f( xLantai,  yLantai,  zLantai );
  glTexCoord2f(0,0);glVertex3f( xLantai, -yLantai,  zLantai );
  glEnd();
...
void loadTexture(){
 	...
 	readFile();
 	texAlas = SOIL_load_OGL_texture
	(
		cJSON_GetObjectItem(objPendopo, "alas")->valuestring,
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
...
void display(){
  ...
  printf("Scale Zoom: %lf\n",zoomOut);
  if (scale <= 6.0 ){
	zoomOut = scale;  	
  	glScalef(scale,zoomOut,scale);
  } else {
  	glScalef(zoomOut,6.0,zoomOut);
  }
